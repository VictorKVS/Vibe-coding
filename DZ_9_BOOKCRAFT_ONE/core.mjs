...
export function optionsFor(s){const p=profiles[s.profile];if(s.step==='intent')return [{label:'Оформить заявку',value:'new'},{label:'Моя история',value:'history'},{label:'Позвать специалиста',value:'human'}];if(s.step==='item')return p.catalog.map(i=>({label:i.name,value:i.id}));if(s.step==='slot')return p.catalog.find(i=>i.id===s.itemId).slots.map(slot=>({label:slot,value:slot}));if(s.step==='confirm')return [{label:'Подтвердить заявку',value:'confirm'},{label:'Изменить выбор',value:'restart'}];if(s.step==='history-result')return [{label:'Оформить новую заявку',value:'restart'},{label:'Позвать специалиста',value:'human'}];return [];}
export function advance(session,input,data){
 ...
 if(s.step==='slot'){if(!p.catalog.find(i=>i.id===s.itemId).slots.includes(raw)){say('Выберите время кнопкой из расписания.');return {session:s};}s.slot=raw;s.step='name';say('Как к вам обращаться?');return {session:s};}
 if(s.step==='name'){if(raw.length<2||raw.length>100){say('Укажите имя длиной от 2 до 100 символов.');return {session:s};}s.name=raw;s.step='contact';say('Укажите email или телефон для связи. Для теста можно использовать client1@example.test.');return {session:s};}
 if(s.step==='contact'){
 ...
 if(s.step==='confirm'){
  if(raw!=='confirm'){say('Нажмите «Подтвердить заявку» или «Изменить выбор». Без подтверждения заявка не создаётся.');return {session:s};}
