...
export function advance(session,input,data){
 ...
 if(raw==='human'||/оператор|администратор|человек|специалист|жалоб/.test(value)){s.step='human';say('Передаю вопрос в демонстрационную очередь специалисту. Контекст разговора сохранён. Внешняя отправка ещё не подключена.');return {session:s};}
