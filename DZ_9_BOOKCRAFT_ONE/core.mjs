...
export function advance(session,input,data){
 ...
 if(s.profile==='medical'&&/^спасибо[.! ]*$/i.test(raw)){say('Пожалуйста. Продолжим запись: '+(s.step==='slot'?'на какой день и время?':s.step==='name'?'как вас зовут?':s.step==='contact'?'оставьте контакт для связи.':'выберите следующий шаг ниже.'));return {session:s};}
