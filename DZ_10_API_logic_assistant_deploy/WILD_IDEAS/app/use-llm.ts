...
export function useLlmGateway(){
 ...
 const ask=useCallback(async({task='dialogue',messages,context}:{task?:LlmTask;messages:{role:'user'|'assistant';content:string}[];context?:Record<string,unknown>})=>{
  setBusy(true);setError('');
  try{
   const r=await fetch('/api/llm',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({selection,task,messages,context})});
