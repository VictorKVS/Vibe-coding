...
22 автоматических теста ядра, UI, голосового ввода и Telegram проверяют все профили, подтверждение заявки, остатки, контакты, изоляцию, историю, отмену и восстановление после ошибки. Интерфейс проверен на ширине 1440 и 390 px. Дашборд вычисляет показатели из демо-записей.

Это **MVP интерфейса и сценарного ядра**, а не подключённый многоканальный AI-сервис. Локальный Whisper распознаёт голос и аудиофайлы. Для отдельного Telegram-бота подготовлены настройка, приём аудио и сценарные ответы; реальный обмен ждёт токена. LLM, телефония и календарь пока не подключены. Учебные обращения не являются реальными покупками или медицинскими записями.
...
[Запуск и настройка отдельного бота](../../DZ_9_BOOKCRAFT_ONE/VOICE_TELEGRAM.md). Три учебные записи озвучены синтезатором и действительно распознаны Whisper; результаты сохранены в проекте. Секретарь отвечает по сценарию без LLM. Токен хранится только локально, в исключённой из Git папке `.runtime`.
