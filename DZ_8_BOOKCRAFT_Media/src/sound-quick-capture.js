...
const STT_STATUS_API = "http://127.0.0.1:8018/api/stt/status";
...
function saveTranscript(text, source, name) {
  ...
  items.push({ id: `audio-${Date.now()}`, source, name, text, created_at: new Date().toISOString() });
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .premium-recorder-health{display:inline-flex;align-items:center;gap:6px;margin-top:8px;padding:5px 8px;border-radius:999px;border:1px solid rgba(95,190,145,.18);background:rgba(60,155,105,.07);color:#9fdcb8;font-size:9px}.premium-recorder-health i{width:6px;height:6px;border-radius:50%;background:currentColor;box-shadow:0 0 10px currentColor}.premium-recorder-health.error{border-color:rgba(255,105,115,.28);background:rgba(190,55,65,.08);color:#ffadb5}.premium-recorder-health.checking{color:#d9c27b;border-color:rgba(220,190,90,.2);background:rgba(190,150,55,.06)}
...
async function refreshSttStatus(popover) {
  const badge = popover.querySelector(".premium-recorder-health");
  if (!badge) return { ready: false, code: "unknown", message: "Статус Whisper неизвестен." };
  badge.className = "premium-recorder-health checking";
  badge.innerHTML = "<i></i> WHISPER · ПРОВЕРКА";
  try {
    const response = await fetch(STT_STATUS_API, { cache: "no-store" });
    const state = response.ok ? await response.json() : { ready: false, code: `http-${response.status}`, message: `STT status HTTP ${response.status}` };
    popover._sttState = state;
    if (state.ready) {
      badge.className = "premium-recorder-health";
      badge.innerHTML = `<i></i> WHISPER READY · ${state.model_name || "model"}`;
    } else {
      badge.className = "premium-recorder-health error";
      badge.innerHTML = `<i></i> STT ERROR · ${state.code || "not-ready"}`;
    }
    badge.title = state.message || "";
    return state;
  } catch {
    const state = { ready: false, code: "gateway-offline", message: "Media Gateway не ответил на проверку Whisper." };
    popover._sttState = state;
    badge.className = "premium-recorder-health error";
    badge.innerHTML = "<i></i> STT ERROR · GATEWAY OFFLINE";
    badge.title = state.message;
    return state;
  }
}
...
function drawIdleWave(popover) {
  ...
  for (let x = 0; x <= rect.width; x += 6) {
    const envelope = .25 + .75 * Math.sin((x / Math.max(1, rect.width)) * Math.PI);
    const amp = (Math.sin(x * .17) + Math.sin(x * .071 + 1.7) * .55) * 11 * envelope;
    const y = center + amp;
...
function updateLevel(popover, level) {
  const bars = Array.from(popover.querySelectorAll(".premium-level-track i"));
  const count = Math.round(Math.max(0, Math.min(1, level)) * bars.length);
  bars.forEach((bar, index) => {
    bar.classList.toggle("on", index < count);
    bar.classList.toggle("hot", index >= Math.floor(bars.length * .78) && index < count);
  });
...
async function transcribe(file, popover, source) {
  ...
  const recognize = popover.querySelectorAll('[data-action="recognize"]');
  const sttState = await refreshSttStatus(popover);
  if (!sttState.ready) {
    transcript.textContent = "Расшифровка не запущена: Whisper не готов.";
    transcript.classList.add("empty");
    setStatus(popover, `STT ERROR · ${sttState.message || sttState.code}`, "error");
    trace("audio.quick.stt.preflight-error", { source, code: sttState.code || "not-ready" });
    return;
  }
  ...
  recognize.forEach((button) => { button.disabled = true; });
  ...
  try {
    ...
    recognize.forEach((button) => { button.disabled = !popover._recorderState?.currentFile; });
  }
}
...
function setPreview(popover, file, source) {
  ...
  audio.load();
  popover.querySelectorAll('[data-action="recognize"]').forEach((button) => { button.disabled = false; });
...
function clearPreview(popover) {
  ...
  audio.load();
  popover.querySelectorAll('[data-action="recognize"]').forEach((button) => { button.disabled = true; });
...
function buildPopover() {
  ...
  popover.innerHTML = `
    <div class="premium-recorder-head">
      <div><span class="kicker">BOOK·CRAFT VOICE STUDIO</span><h3>Премиум-диктофон</h3><p>Запись → прослушивание → локальный Whisper → текст произведения</p><span class="premium-recorder-health checking"><i></i> WHISPER · ПРОВЕРКА</span></div>
      <button type="button" aria-label="Закрыть">×</button>
    </div>
    <section class="premium-recorder-stage">
      <div class="premium-wave-wrap"><canvas data-recorder="wave"></canvas><div class="premium-wave-grid"></div><div class="premium-wave-center"></div><div class="premium-timer"><span data-recorder="timer">00:00</span><small data-recorder="state">готов к записи</small></div><div class="premium-file-meta"><strong data-recorder="file-name">Новая запись</strong><span data-recorder="file-meta">локальная запись</span></div></div>
      <div class="premium-level"><label>Уровень голоса</label><div class="premium-level-track">${Array.from({ length: 24 }, () => "<i></i>").join("")}</div><output data-recorder="level">0%</output></div>
    </section>
    <section class="premium-transport">
      <div class="premium-side-controls"><button type="button" data-action="pause" disabled>Ⅱ Пауза</button><button type="button" data-action="upload-audio">↑ MP3 / аудио</button><input data-sound="quick-file" type="file" accept="audio/mpeg,audio/mp3,audio/wav,audio/x-wav,audio/mp4,audio/x-m4a,audio/ogg,audio/webm,.mp3,.wav,.m4a,.ogg,.webm" hidden /></div>
      <div class="premium-record-wrap"><button type="button" class="premium-record-button" data-action="record" aria-label="Начать запись"></button><div class="premium-record-caption" data-recorder="caption">Начать запись</div></div>
      <div class="premium-side-controls right"><button type="button" data-action="recognize" disabled>✦ Распознать</button><button type="button" data-action="sound-tab">Sound Engineering</button></div>
    </section>
    <section class="premium-preview"><audio data-recorder="preview" controls preload="metadata"></audio><div class="premium-preview-actions"><button type="button" class="primary" data-action="recognize">✦ Распознать Whisper</button><button type="button" class="danger" data-action="delete-audio">Удалить запись</button></div></section>
    <section class="premium-transcription"><div class="premium-transcription-head"><strong>Расшифровка</strong><span>проверяйте текст перед вставкой</span></div><div class="bookcraft-sound-transcript empty">После распознавания здесь появится текст.</div><div class="premium-insert-row"><select data-sound="target"><option value="whole">Добавить как рассказ целиком</option><option value="scene">Добавить как новую сцену</option><option value="introduction">Во вступление</option><option value="development">В развитие</option><option value="finale">В финал</option></select><button type="button" class="primary" data-action="insert" disabled>Вставить в произведение</button></div><div class="bookcraft-sound-quick-status">Готов к новой записи.</div></section>
  ...
  popover._recorderState = { recorder: null, stream: null, chunks: [], startedAt: 0, elapsedBeforePause: 0, timer: null, audioContext: null, analyser: null, animationFrame: null, currentFile: null, currentSource: "", previewUrl: "" };
  popover._sttState = null;
  ...
  popover.querySelector('[data-action="delete-audio"]')?.addEventListener("click", () => { clearPreview(popover); trace("audio.quick.preview.delete", {}); });
  drawIdleWave(popover);
  refreshSttStatus(popover);
  return popover;
}
...
function startClock(popover) { const state = popover._recorderState; if (!state) return; clearInterval(state.timer); updateRecorderClock(popover); state.timer = window.setInterval(() => updateRecorderClock(popover), 250); }
function stopClock(popover) { const state = popover._recorderState; if (!state) return; clearInterval(state.timer); state.timer = null; updateRecorderClock(popover); }

async function startRecording(popover, externalButton = null) {
  const state = popover._recorderState;
  if (!state) return;
  if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === "undefined") { setStatus(popover, "Браузер не поддерживает запись с микрофона.", "error"); return; }
  try {
    ...
    state.stream = stream; state.recorder = recorder; state.chunks = []; state.startedAt = Date.now(); state.elapsedBeforePause = 0;
    recorder.ondataavailable = (event) => { if (event.data?.size) state.chunks.push(event.data); };
    recorder.onstop = () => {
      stopClock(popover); stopVisualizer(popover); state.stream?.getTracks?.().forEach((track) => track.stop());
      ...
      popover.querySelector('[data-recorder="caption"]').textContent = "Записать ещё";
      popover.querySelector('[data-recorder="state"]').textContent = "запись готова";
      popover.querySelector('[data-action="pause"]').disabled = true;
      if (externalButton) { externalButton.classList.remove("recording"); externalButton.textContent = "🎙 Диктовать"; }
      setStatus(popover, `Запись готова · ${formatBytes(file.size)}. Прослушайте и нажмите «Распознать Whisper».`, "ready");
      trace("audio.quick.record.finish", { size_bytes: file.size, content_type: finalMime, duration_ms: state.elapsedBeforePause });
      state.recorder = null; state.stream = null; state.chunks = []; state.startedAt = 0;
    };
    recorder.start(500);
    startVisualizer(popover, stream); startClock(popover);
    const recordButton = popover.querySelector('[data-action="record"]'); recordButton.classList.add("recording"); recordButton.setAttribute("aria-label", "Остановить запись");
    popover.querySelector('[data-recorder="caption"]').textContent = "Остановить"; popover.querySelector('[data-recorder="state"]').textContent = "идёт запись"; popover.querySelector('[data-action="pause"]').disabled = false;
    if (externalButton) { externalButton.classList.add("recording"); externalButton.textContent = "■ Остановить"; }
  ...
  } catch (error) { setStatus(popover, "Не удалось включить микрофон. Разрешите доступ браузеру.", "error"); trace("audio.quick.record.error", { error: String(error.message || error).slice(0, 220) }); }
}

function stopRecording(popover) {
  const state = popover._recorderState;
  if (state?.recorder?.state === "recording" || state?.recorder?.state === "paused") { if (state.startedAt) state.elapsedBeforePause += Date.now() - state.startedAt; state.startedAt = 0; state.recorder.stop(); }
}

function togglePause(popover) {
  const state = popover._recorderState; const button = popover.querySelector('[data-action="pause"]'); const label = popover.querySelector('[data-recorder="state"]'); if (!state?.recorder) return;
  if (state.recorder.state === "recording") { state.elapsedBeforePause += Date.now() - state.startedAt; state.startedAt = 0; state.recorder.pause(); button.classList.add("active"); button.textContent = "▶ Продолжить"; label.textContent = "пауза"; stopClock(popover); trace("audio.quick.record.pause", { duration_ms: state.elapsedBeforePause }); }
  else if (state.recorder.state === "paused") { state.recorder.resume(); state.startedAt = Date.now(); button.classList.remove("active"); button.textContent = "Ⅱ Пауза"; label.textContent = "идёт запись"; startClock(popover); trace("audio.quick.record.resume", { duration_ms: state.elapsedBeforePause }); }
}
...
function buildQuickControls(root) {
  ...
  const quick = document.createElement("div"); quick.className = "bookcraft-sound-quick"; quick.innerHTML = `<button type="button" data-action="quick-mic">🎙 Диктовать</button><button type="button" data-action="quick-file">⬆ Рассказ MP3</button>`; underhood?.insertAdjacentElement("beforebegin", quick);
  const popover = buildPopover();
  const embeddedInput = popover.querySelector('[data-sound="quick-file"]'); const fileButton = quick.querySelector('[data-action="quick-file"]'); const micButton = quick.querySelector('[data-action="quick-mic"]'); const recordButton = popover.querySelector('[data-action="record"]'); const pauseButton = popover.querySelector('[data-action="pause"]'); const uploadButton = popover.querySelector('[data-action="upload-audio"]');
  fileButton?.addEventListener("click", () => { popover.classList.add("open"); refreshSttStatus(popover); embeddedInput?.click(); });
  uploadButton?.addEventListener("click", () => embeddedInput?.click());
  embeddedInput?.addEventListener("change", () => { const file = embeddedInput.files?.[0]; embeddedInput.value = ""; if (!file) return; popover.classList.add("open"); clearPreview(popover); setPreview(popover, file, "file"); setStatus(popover, `Аудиофайл загружен · ${formatBytes(file.size)}. Прослушайте или нажмите «Распознать Whisper».`, "ready"); trace("audio.quick.file.ready", { size_bytes: file.size, content_type: file.type || "unknown" }); });
  micButton?.addEventListener("click", async () => { popover.classList.add("open"); refreshSttStatus(popover); const state = popover._recorderState; if (state?.recorder?.state === "recording" || state?.recorder?.state === "paused") stopRecording(popover); else await startRecording(popover, micButton); });
  recordButton?.addEventListener("click", async () => { const state = popover._recorderState; if (state?.recorder?.state === "recording" || state?.recorder?.state === "paused") stopRecording(popover); else await startRecording(popover, micButton); });
  pauseButton?.addEventListener("click", () => togglePause(popover));
}

function enhanceWorkspace() { const root = document.querySelector(".workspace-shell"); if (!root) return; buildQuickControls(root); }
export function mountSoundQuickCapture() { injectStyles(); const observer = new MutationObserver(enhanceWorkspace); observer.observe(document.documentElement, { childList: true, subtree: true }); enhanceWorkspace(); }
