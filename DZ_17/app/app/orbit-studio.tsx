...
import {ArrowRight,ArrowLeft,Check,Mic,Square,Volume2,VolumeX,Maximize2,ImagePlus,X,ScanSearch} from 'lucide-react';
import {Button} from '@/components/ui/button';
import {Dialog,DialogContent,DialogTitle,DialogDescription} from '@/components/ui/dialog';
import {interview,worlds,heroes,looks} from './story-data';
import {useLlmGateway,type LlmImage} from './use-llm';
...
export function OrbitStudio(p:Props){
 const [hero,setHero]=useState(0),[preview,setPreview]=useState<number|null>(null),[aiReply,setAiReply]=useState(''),[visual,setVisual]=useState<LlmImage|null>(null),[visualError,setVisualError]=useState(''),[lastPrompt,setLastPrompt]=useState('');
 ...
 const attachImage=(file?:File)=>{
  ...
  reader.onload=()=>{if(typeof reader.result==='string')setVisual({dataUrl:reader.result,name:file.name});else setVisualError('Не удалось прочитать изображение.');};
  reader.onerror=()=>setVisualError('Не удалось прочитать изображение.');
 ...
 const submitIdea=async(answer:string)=>{
  ...
  setAiReply('');setLastPrompt(clean);p.choose(clean);
  try{
   const text=await llm.ask({task:visual?'vision':'dialogue',messages:[{role:'user',content:clean}],images:visual?[visual]:[],context:{stage:'idea_intake',confirmedAnswers:nextAnswers,nextQuestion,multimodal:Boolean(visual),visualInstruction:visual?'Ответь по структуре: 1) НАБЛЮДЕНИЯ — только то, что действительно видно; 2) ИНТЕРПРЕТАЦИЯ — как это связано с задачей; 3) ТРИ НАПРАВЛЕНИЯ — три разных творческих применения, кратко сильная сторона и риск каждого. Не придумывай невидимые детали.':'Изображение не приложено.',rule:visual?'Не принимай решение за пользователя. После трёх направлений естественно перейди к следующему вопросу.':'Не принимай решение за пользователя. Кратко отрази смысл ответа и перейди к следующему вопросу.'}});
   setAiReply(text);
  }catch{setAiReply('');}
 };
 ...
 const speech=p.stage===2?`${p.names[hero]||heroes[hero].name}. ${heroes[hero].detail} Выберите образ — его можно рассмотреть крупнее.`:p.stage===0&&llm.busy?'Анализирую текст и визуальный контекст проекта…':p.stage===0&&aiReply?aiReply:p.line;
 return <section className="orbital-studio" aria-label="Круговой диалог с Алиной">
  ...
  <div className={'orbit-field '+(p.stage>0?'with-images':'')}>
   ...
   <div className={'orbit-core '+(p.speaking?'is-speaking':'')+(p.listening?' is-listening':'')+(llm.busy?' is-thinking':'')}><div className="core-face"><img src="/alina.png" alt="Алина — цифровая ведущая"/></div><strong>АЛИНА</strong><span>{p.listening?'Слушаю ваш ответ':llm.busy?'Смотрю и думаю…':p.speaking?'Рассказываю':'Создаём вашу историю'}</span><div className="core-controls"><Button variant="ghost" size="icon" aria-label={p.voiceOn?'Выключить голос':'Включить голос'} onClick={p.toggleVoice}>{p.voiceOn?<VolumeX/>:<Volume2/>}</Button><Button variant="ghost" size="icon" aria-label="Повторить реплику" onClick={p.repeat}><Volume2/></Button>{p.stage===0&&q<3&&<Button variant="ghost" size="icon" aria-label={p.listening?'Остановить микрофон':'Ответить голосом'} onClick={p.microphone}>{p.listening?<Square/>:<Mic/>}</Button>}</div></div>
   <div className="orbit-speech glass" aria-live="polite"><span className="label">АЛИНА / {p.stage===0?'MULTIMODAL LLM':p.stage===1?'ВАШ МИР':'ПЕРСОНАЖИ'}</span>{p.stage===0&&visual&&lastPrompt&&<div className="visual-request"><b>TEXT QUERY</b><span>{lastPrompt}</span></div>}<p>{speech}</p>{p.stage===0&&visual&&<small className="visual-trace"><ScanSearch size={13}/> IMAGE + TEXT · {visual.name||'visual reference'}</small>}{p.stage===0&&llm.meta&&<small className="llm-trace">MODEL {llm.meta.model} · {llm.meta.latencyMs} ms · {llm.meta.task}</small>}</div>
   ...
   <div className="orbit-memory" aria-label="Выбранные решения">{p.answers.map((a,i)=><span key={i}><Check size={12}/>{a}</span>)}{visual&&<span><ImagePlus size={12}/>Визуальный референс</span>}{p.world&&<span><Check size={12}/>{worlds.find(w=>w.id===p.world)?.name}</span>}</div>
  </div>
  {p.error&&<p className="orbit-error" role="status">{p.error}</p>}
  {p.stage===0&&q<3&&<form className="orbit-compose glass multimodal-compose" onSubmit={e=>{e.preventDefault();if(p.draft.trim())void submitIdea(p.draft);}}>
   <div className="visual-input-row"><label className="visual-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>attachImage(e.target.files?.[0])}/><ImagePlus size={18}/><span>{visual?'Заменить фото':'Добавить фото'}</span></label>{visual&&<div className="visual-preview"><img src={visual.dataUrl} alt="Загруженный визуальный референс"/><div><strong>{visual.name||'visual-reference'}</strong><span>будет анализироваться вместе с текстом</span></div><button type="button" aria-label="Удалить изображение" onClick={()=>{setVisual(null);setVisualError('');setLastPrompt('');}}><X size={16}/></button></div>}</div>
   {visualError&&<p className="visual-error" role="status">{visualError}</p>}
   <label htmlFor="orbit-answer">Текстовый запрос</label><input id="orbit-answer" placeholder={visual?'Например: что из этого референса можно использовать в моей сцене?':'Скажите или напишите свою идею…'} value={p.draft} onChange={e=>p.setDraft(e.target.value)} maxLength={1000}/><Button className="outline" variant="outline" type="button" onClick={p.microphone}>{p.listening?<Square/>:<Mic/>}{p.listening?'Стоп':'Голос'}</Button><Button className="primary" type="submit" disabled={!p.draft.trim()||llm.busy}>{visual?<ScanSearch/>:null}{visual?'Анализировать':'Подтвердить'} <ArrowRight/></Button><small>Для ДЗ-17 можно загрузить фото и одновременно задать текстовый вопрос. Файл уходит выбранной vision-модели через серверный gateway; API-ключ в браузер не передаётся.</small>
  </form>}
