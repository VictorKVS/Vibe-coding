...
export function OrbitStudio(p:Props){
 const [hero,setHero]=useState(0),[preview,setPreview]=useState<number|null>(null),[aiReply,setAiReply]=useState(''),[visual,setVisual]=useState<LlmImage|null>(null),[visualError,setVisualError]=useState('');
 ...
 return <section className="orbital-studio" aria-label="Круговой диалог с Алиной">
  ...
  <div className={'orbit-field '+(p.stage>0?'with-images':'')}>
   ...
   <div className="orbit-speech glass" aria-live="polite"><span className="label">АЛИНА / {p.stage===0?'MULTIMODAL LLM':p.stage===1?'ВАШ МИР':'ПЕРСОНАЖИ'}</span><p>{speech}</p>{p.stage===0&&visual&&<small className="visual-trace"><ScanSearch size={13}/> IMAGE + TEXT · {visual.name||'visual reference'}</small>}{p.stage===0&&llm.meta&&<small className="llm-trace">MODEL {llm.meta.model} · {llm.meta.latencyMs} ms · {llm.meta.task}</small>}</div>
