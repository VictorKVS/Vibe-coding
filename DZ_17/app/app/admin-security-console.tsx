   ...
   <aside className="system-console" aria-label="Центр управления ALINA">
    ...
    <div className="system-console-body">
     ...
     <section className="system-content">
      ...
      <div className="system-auth-card"><div><strong>{role==='admin'?'ADMIN TOKEN':'SECURITY TOKEN'}</strong><small>{roleConfigured?'серверная роль настроена':'серверная роль не настроена'}</small></div><input type="password" value={token} onChange={e=>setToken(e.target.value)} placeholder={roleConfigured?'Введите токен роли':'Сначала задайте токен в .env.local'} autoComplete="off"/><input value={reason} onChange={e=>setReason(e.target.value)} placeholder="Причина изменения"/></div>
      {error?<div className="system-alert error">{error}</div>:null}
      {tab==='models'?<ModelsPanel models={models} routes={routes} online={online} providers={providers} role={role} config={config} mutate={mutate}/>:null}
...
function ModelsPanel({models,routes,online,providers,role,config,mutate}:{models:Model[];routes:Record<string,string[]>;online:number;providers:string[];role:Role;config:Config|null;mutate:(a:string,t:string,v:unknown)=>Promise<void>}){
 const control=config?.control;const giga=models.filter(m=>m.provider==='gigachat');const tasks=config?.routableTasks||['dialogue','synthesis','architecture','kb_extract','kb_validate'];
