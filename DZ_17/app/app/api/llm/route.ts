...
async function callCompatible(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[]){
 ...
 const r=await fetch(`${base}/chat/completions`,{method:'POST',headers,body:JSON.stringify({model,messages:[{role:'system',content:systemFor(task)},{role:'user',content:userContent}],temperature:task.startsWith('kb_')?.2:.65,max_tokens:task.startsWith('kb_')?2600:900})});
...
async function callOllama(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[]){
 ...
 const r=await fetch(`${base}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,stream:false,messages:[{role:'system',content:systemFor(task)},userMessage],options:{temperature:task.startsWith('kb_')?.2:.65}})});const data=await r.json().catch(()=>({})) as {error?:string;message?:{content?:string}};if(!r.ok)throw new Error(data?.error||`Ollama HTTP ${r.status}`);const text=data?.message?.content?.trim();if(!text)throw new Error('Ollama вернула пустой ответ.');return {text,usage:null};
}
...
export async function GET(){
 const routes:Object=Object.fromEntries((['dialogue','synthesis','architecture','vision','kb_extract','kb_validate'] as Task[]).map(task=>[task,autoCandidates(task).map(x=>x.id)]));
