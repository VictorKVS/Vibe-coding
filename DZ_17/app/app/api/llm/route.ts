...
import {effectivePromptForTask,type PromptTask} from '@/lib/prompt-registry';
import {enrichTranslationContext} from '@/lib/translator-rag';

type Task=PromptTask;
...
function effectiveCatalog(runtime:RuntimeCatalog,policy:RuntimePolicyState){
 return catalog(runtime).map(item=>{
  if(item.id==='auto')return item;
  const decision=modelPolicyDecision(policy,item.id);
...
async function callLlama(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[],systemPrompt:string){
 const prompt=dialogueText(messages,context);const r=await fetch(`${llamaBase()}/v1/chat/completions`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,messages:[{role:'system',content:systemPrompt},{role:'user',content:openAIStyleContent(prompt,images)}],temperature:taskTemperature(task),max_tokens:taskMaxTokens(task)})});
...
async function callGiga(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[],systemPrompt:string){
 ...
 const token=await getGigaToken();const prompt=dialogueText(messages,context);const r=await fetch(`${gigaBase()}/v1/chat/completions`,{method:'POST',headers:{'content-type':'application/json','accept':'application/json','authorization':`Bearer ${token}`},body:JSON.stringify({model,messages:[{role:'system',content:systemPrompt},{role:'user',content:prompt}],temperature:taskTemperature(task)})});
...
async function callOpenAI(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[],systemPrompt:string){
 const text=dialogueText(messages,context);const input=images.length?[{role:'user',content:[{type:'input_text',text},...images.map(img=>({type:'input_image',image_url:img.dataUrl}))]}]:text;const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${env('OPENAI_API_KEY')}`},body:JSON.stringify({model,instructions:systemPrompt,input,max_output_tokens:taskMaxTokens(task)})});
...
async function callOllama(model:string,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[],systemPrompt:string){
 const base=(env('OLLAMA_BASE_URL')||'http://127.0.0.1:11434').replace(/\/$/,'');const prompt=dialogueText(messages,context);const ollamaImages=images.map(img=>img.dataUrl.split(',',2)[1]).filter(Boolean);const userMessage:Record<string,unknown>={role:'user',content:prompt};if(ollamaImages.length)userMessage.images=ollamaImages;const r=await fetch(`${base}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,stream:false,messages:[{role:'system',content:systemPrompt},userMessage],options:{temperature:taskTemperature(task)}})});const data=await r.json().catch(()=>({})) as {error?:string;message?:{content?:string}};if(!r.ok)throw new Error(data?.error||`Ollama HTTP ${r.status}`);const text=data?.message?.content?.trim();if(!text)throw new Error('Ollama вернула пустой ответ.');return {text,usage:null};
}
...
async function call(item:ModelItem,task:Task,messages:Message[],context:Record<string,unknown>|undefined,images:ImageInput[],systemPrompt:string){if(item.provider==='llamacpp')return callLlama(item.model,task,messages,context,images,systemPrompt);if(item.provider==='gigachat')return callGiga(item.model,task,messages,context,images,systemPrompt);if(item.provider==='openai')return callOpenAI(item.model,task,messages,context,images,systemPrompt);if(item.provider==='compatible')return callCompatible(item.model,task,messages,context,images,systemPrompt);if(item.provider==='ollama')return callOllama(item.model,task,messages,context,images,systemPrompt);return demoReply(task,messages,context);}
...
export async function POST(request:Request){
 ...
 try{
  ...
  const [runtime,policy,prompt]=await Promise.all([runtimeCatalog(),readRuntimePolicy(),effectivePromptForTask(task)]);
  const promptId=promptIdForTask(task);if(prompt.id!==promptId)throw new Error(`Prompt registry mismatch: policy=${promptId}, runtime=${prompt.id}`);const promptDecision=promptPolicyDecision(policy,promptId);if(!promptDecision.allowed)return Response.json({error:`Prompt policy denied ${promptId}: ${promptDecision.reason}`,code:'PROMPT_POLICY_DENY',policyVersion:policy.version},{status:403});
  ...
  if(task==='translation'){
   ...
   if(exact?.target)return Response.json({text:exact.target,usage:null,selection:'translation-memory:exact',provider:'translation-memory',model:'exact-match',task,imagesReceived:0,latencyMs:Date.now()-started,attempts:[],policyVersion:policy.version,promptId,promptVersion:prompt.version,kbRefs,routeOverride:null,translationRag:rag});
  }
  ...
  for(const item of candidates){try{const result=await call(item,task,messages,effectiveContext,images,prompt.body);return Response.json({...result,selection:item.id,provider:item.provider,model:item.model,task,imagesReceived:images.length,latencyMs:Date.now()-started,attempts,policyVersion:policy.version,promptId,promptVersion:prompt.version,kbRefs,routeOverride:routeOverrideForTask(policy,task)||null,translationRag:task==='translation'?effectiveContext?.translationRag:null});}catch(error){attempts.push({model:item.id,error:error instanceof Error?error.message:'provider error'});if(body.selection&&body.selection!=='auto')throw error;}}
