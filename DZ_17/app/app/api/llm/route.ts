...
type RequestBody={selection?:string;task?:Task;messages?:Message[];context?:Record<string,unknown>};
...
async function callOpenAI(model:string,messages:Message[],context?:Record<string,unknown>){
 const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${env('OPENAI_API_KEY')}`},body:JSON.stringify({model,instructions:ALINA_SYSTEM,input:dialogueText(messages,context),max_output_tokens:900})});
 const data:any=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(data?.error?.message||`OpenAI HTTP ${r.status}`);
 const text=extractOpenAI(data);if(!text)throw new Error('OpenAI вернул пустой ответ.');
 return {text,usage:data?.usage||null};
}
async function callCompatible(model:string,messages:Message[],context?:Record<string,unknown>){
 ...
 const r=await fetch(`${base}/chat/completions`,{method:'POST',headers,body:JSON.stringify({model,messages:[{role:'system',content:ALINA_SYSTEM},...messages,{role:'user',content:contextText(context)||'Продолжай с учётом контекста.'}],temperature:.65,max_tokens:900})});
 const data:any=await r.json().catch(()=>({}));
...
async function callOllama(model:string,messages:Message[],context?:Record<string,unknown>){
 const base=(env('OLLAMA_BASE_URL')||'http://127.0.0.1:11434').replace(/\/$/,'');
 const r=await fetch(`${base}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,stream:false,messages:[{role:'system',content:ALINA_SYSTEM},...messages,{role:'user',content:contextText(context)||'Продолжай.'}]})});
 const data:any=await r.json().catch(()=>({}));
...
function demoReply(messages:Message[],context?:Record<string,unknown>){
 ...
 return {text:`Я услышала: ${last.slice(0,220)}. Зафиксирую это как рабочую гипотезу и не буду менять без вашего подтверждения.${next}`,usage:null};
}
...
export async function POST(request:Request){
 ...
 try{
  ...
  const result=item.provider==='openai'?await callOpenAI(item.model,messages,body.context):item.provider==='compatible'?await callCompatible(item.model,messages,body.context):item.provider==='ollama'?await callOllama(item.model,messages,body.context):demoReply(messages,body.context);
  return Response.json({...result,selection:item.id,provider:item.provider,model:item.model,task,latencyMs:Date.now()-started});
