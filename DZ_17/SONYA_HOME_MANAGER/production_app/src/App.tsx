...
type Workspace = 'analyze' | 'plan' | 'shopping';
...
export default function App() {
  ...
  const [workspace, setWorkspace] = useState<Workspace>('analyze');
  ...
  return <div className="app-shell" style={style}>
    <header className="topbar glass">
      <button className="brand" onClick={() => setWorkspace('analyze')}><Home size={22}/><span><b>СОНЯ</b><small>ВАШ ДОМ В ГАРМОНИИ</small></span></button>
      <nav><button onClick={() => setWorkspace('analyze')}>Главная</button><button onClick={() => applyPreset('menu')}>Меню</button><button onClick={() => applyPreset('kids')}>Детский обед</button><button onClick={() => setWorkspace('plan')}>Праздники</button><button onClick={() => setWorkspace('shopping')}>Покупки</button></nav>
    ...
    <section className="hero glass" onMouseEnter={() => design.pauseOnHover && setHeroPaused(true)} onMouseLeave={() => setHeroPaused(false)}>
    ...
    <main className="workspace">
      <aside className="sidebar glass"><Side icon={<Home/>} title="Мой дом" onClick={() => setWorkspace('analyze')}/><Side icon={<UtensilsCrossed/>} title="Планировщик меню" onClick={() => applyPreset('menu')}/><Side icon={<Sparkles/>} title="Детский обед" onClick={() => applyPreset('kids')}/><Side icon={<ChefHat/>} title="Десерты без сахара" onClick={() => applyPreset('dessert')}/><Side icon={<Volume2/>} title="Компоты и морсы" onClick={() => applyPreset('drinks')}/><Side icon={<PartyPopper/>} title="Праздники и гости" onClick={() => setWorkspace('plan')}/><Side icon={<ShoppingBasket/>} title="Покупки" onClick={() => setWorkspace('shopping')}/><Side icon={<WandSparkles/>} title="Полезные советы" onClick={() => applyPreset('tips')}/><Side icon={<Refrigerator/>} title="Мой холодильник" onClick={() => applyPreset('fridge')}/><div className="mini"><CreditCard size={16}/> Счета · soon</div></aside>
